---
title: '```rlaqn```: Data retrieval from the London Air API'
author: "Justin Lingard : [prejjnl@gmail.com](prejjnl@gmail.com)"
date: ""
output:
  html_document:
    code_folding: hide
...
```{r, eval=FALSE, results="asis", echo=FALSE, warning=FALSE}
# Table 1
theData <- arrange(select(theData, SiteCode, SiteName, Year, SpeciesCode, Value, Achieved), SiteCode, Year)
theData$SpeciesCode <- gsub(pattern = "NO2", replacement = "NO<sub>2</sub>", theData$SpeciesCode)
pander::pandoc.table(theData, split.table = Inf,  style = 'rmarkdown', justify = c('left'), caption = paste0("Exceedances of hourly NO<sub>2</sub> air quality objective per year"))
```
...
#  Select theSites
theSites <- laqn_sites %>% filter(SiteCode == "MY1" | SiteCode == "WM6" | SiteCode == "WA7" | SiteCode == "WA8")
# Load the leaflet and mapshot packages
library(leaflet)
m <- leaflet() %>%
setView(lng = -0.17, lat = 51.5, zoom = 12) %>%
addTiles() %>%
